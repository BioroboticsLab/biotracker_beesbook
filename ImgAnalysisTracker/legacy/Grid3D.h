...
class Grid3D : public BioTracker::Core::ObjectModel {
  public:
    ...
    static_assert(POINTS_PER_RING % 4 == 0 ,
                  "POINTS_PER_RING = NUM_MIDDLE_CELLS * POINTS_PER_MIDDLE_CELL must be a multiple of 4");
    ...
    double  getPixelRadius() const {
        return _radius / FOCAL_LENGTH;
    }
    ...
    boost::tribool hasBeenBitToggled() const {
        return _bitsTouched;
    }
    void setBeenBitToggled(const boost::logic::tribool::value_t toggled) {
        _bitsTouched.value = toggled;
    }
    ...
    std::vector<cv::Point> const &getOuterRingPoints() const {
        return _coordinates2D[OUTER_RING];
    }
  ...
  private:
    ...
    struct coordinates_t {
        ...
        coordinates_t(coordinates_t &&rhs) : _rings(std::move(rhs._rings)), _inner_line(std::move(rhs._inner_line)),
            _inner_ring(_rings[INNER_RING]), _middle_ring(_rings[MIDDLE_RING]), _outer_ring(_rings[OUTER_RING]) {}
    ...
    double
    _angle_z;           // the angle of the grid (unit: rad. points towards the head of the bee, positive is counter-clock)
    double
    _angle_y;           // the rotation angle of the grid around y axis (rotates into z - space)
    double
    _angle_x;           // the rotation angle of the grid around x axis (rotates into z - space)
    idarray_t
    _ID;                // bit pattern of tag (false and true for black and white, indeterminate for unrecognizable)
    std::vector<std::vector<cv::Point>>
                                     _coordinates2D;     // 2D coordinates of mesh (after perspective projection) (see opencv function drawContours)
    std::vector<cv::Point>
    _interactionPoints; // 2D coordinates of interaction points (center of grid, grid cell centers, etc)
    ...
    boost::tribool
    _bitsTouched;       // if at least one bit was set, this is true, after copy & paste indeterminate
