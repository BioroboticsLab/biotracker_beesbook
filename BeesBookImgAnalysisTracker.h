...
private:
	...
	struct LocalizerEvaluationResults {
		...
		std::map<std::reference_wrapper<const decoder::Tag>, std::shared_ptr<InteractiveGrid>> gridByTag;
	};
	...
	std::pair<double, std::reference_wrapper<const decoder::TagCandidate>> compareGrids(const decoder::Tag& detectedTag, std::shared_ptr<InteractiveGrid> const& grid) const;
