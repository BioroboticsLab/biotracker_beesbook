	...
	, _visualizeFrames(true)
{
	...
	// and x axis
	double theta = 0;
	// rotation is the rotation of the grid cells
	double rotation = 0;

    for (;;) {
        const double theta_rad    = theta * CV_PI / 180.;
        ...
        // TODO: +/- ellipse_roll
        const auto rotationMatrix = CvHelper::rotationMatrix_z(theta_rad) * CvHelper::rotationMatrix_x(-ellipse_roll) * CvHelper::rotationMatrix_z(rotation);
        ...
        image = cv::Scalar(128);
        cv::ellipse(image, cen, axes, theta, 0, 360, cv::Scalar(255), -1);
        cv::circle(image, cen, 2, cv::Scalar(0), 1);
        const double y1 = major * std::sin(theta_rad);
        const double x1 = major * std::cos(theta_rad);
        const double angle_minor = CV_PI - (theta_rad + (CV_PI / 2));
		...
		Grid debugGrid(cv::Point2i(0, 0), major, angle_z, angle_y, angle_x);
		debugGrid.debugDraw("1");
