...
void BeesBookTagMatcher::mouseReleaseEvent(QMouseEvent *e) {
    ...
    if (e->button() == Qt::LeftButton) {
        ...
        case State::SetTag: {
            ...
            _activeGrid = std::make_shared<InteractiveGrid>(_orient.from, static_cast<double>(GRID_RADIUS_PIXELS), _orient.alpha(),
                          0., 0.);
...
void BeesBookTagMatcher::removeCurrentActiveTag() {
    ...
    [&](const TrackedObject & o) {
        return o.getId() == _activeGridObjectId.get();
    }) ;
